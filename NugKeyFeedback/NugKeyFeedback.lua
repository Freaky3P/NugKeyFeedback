...
local COOLDOWN_SHOW_DELAY = 3

local function GetSpellID(spellName)
    local id = ns.ResolveSpell(spellName)
    return id
end

local lastCast = { name = nil, texture = nil, t = 0 }
...
function NugKeyFeedback:PLAYER_LOGIN(event)
    ...
    self.mirror.UpdateCooldownOrCast = function(self)
        ...
        local cooldownStartTime, cooldownDuration, enable = GetActionCooldown(action)
        ...
        if NugKeyFeedback.db.enableCast and self.castSpellID and self.castSpellID == GetActionSpellID(action) and castDuration > cooldownDuration then
            ...
            CooldownFrame_SetTimer(cooldownFrame, self.castStartTime, castDuration, 1)
        elseif NugKeyFeedback.db.enableCooldown and cooldownDuration and cooldownDuration > 0
                and cooldownStartTime and (GetTime() - cooldownStartTime) >= COOLDOWN_SHOW_DELAY then
    ...
    ns.BuildSpellData()
    ns.onItemUsed = function(texture) NugKeyFeedback:ShowLineIcon(texture) end
    ns.showMirror = function(texture, cdStart, cdDur) NugKeyFeedback:ShowMirrorTexture(texture, cdStart, cdDur) end
    ns.SetupItemHooks()
...
function NugKeyFeedback:COMPANION_LEARNED(event)
    ns.BuildSpellData()
end
...
function NugKeyFeedback.UNIT_SPELLCAST_START(self,event, unit, _castID)
    ...
    if texture and (GetTime() - (ns.lastItemUseTime or 0)) > 0.5 then

        lastCast.name, lastCast.texture, lastCast.t = (name or _castID), texture, GetTime()
    else
        lastCast.name = nil
    end
...
function NugKeyFeedback.UNIT_SPELLCAST_INTERRUPTED(self, event, unit, castID)
    if unit ~= "player" then
        return
    end
    ns.ClearPending()
    NugKeyFeedback.UNIT_SPELLCAST_STOP(self, event, unit, castID)
end
...
function NugKeyFeedback:UNIT_INVENTORY_CHANGED(event, unit)
    if unit and unit ~= "player" then return end
    local tex = ns.ConfirmEquipUse()
    if not tex then return end
    if self.db.enableCastLine then self:ShowLineIcon(tex) end
...
function NugKeyFeedback:ShowMirrorTexture(texture, cdStart, cdDur)
    if not texture then return end
    local mirror = self.mirror
    mirror.action = nil
    mirror.castSpellID = nil
    mirror.castDuration = nil
    mirror.icon:SetTexture(texture)
    if self.db.enableCooldown and cdStart and cdDur and cdDur > 0
            and (GetTime() - cdStart) >= COOLDOWN_SHOW_DELAY then
        mirror.cooldown:SetDrawEdge(false)
        mirror.cooldown:SetReverse(false)
        CooldownFrame_SetTimer(mirror.cooldown, cdStart, cdDur, 1)
    else
...
function NugKeyFeedback:ShowLineIcon(texture)
    if not texture then return end
    if not (self.db and self.db.enableCastLine and self.iconPool) then return end
...
function NugKeyFeedback:UNIT_SPELLCAST_SUCCEEDED(event, unit, spellName)
    ...
    local function flashLine(texture)
        if texture and self.db.enableCastLine then self:ShowLineIcon(texture) end
        if self.db.enableCastFlash then
            self.mirror.glow:Show()
            self.mirror.glow.blink:Play()
        end
    ...
    local spellID = ns.ResolveSpell(spellName)
    if spellID then
        if spellID == 75 then return end
        if IsSpellKnown(spellID, BOOKTYPE_SPELL) or IsSpellKnown(spellID, BOOKTYPE_PET)
            or ns.trackedIDs[spellID] then
            ns.ClearPending()
            return flashLine(select(3, GetSpellInfo(spellID)))
        end
    end

    local itemTex = ns.ConfirmItemUse()
    if itemTex then
        return flashLine(itemTex)
    end

    if lastCast.texture and lastCast.name == spellName and (GetTime() - lastCast.t) < 15 then
        return flashLine(lastCast.texture)
    end
end
