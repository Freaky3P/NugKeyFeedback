...
function NugKeyFeedback:CreateLastSpellIconLine(anchorTo)

    local hdr = CreateFrame("Frame", "NugKeyFeedbackLine", self)
    hdr:SetAllPoints(anchorTo)
    hdr:SetFrameLevel(anchorTo:GetFrameLevel() + 1)
    hdr:Show()

    local template = nil
    local resetterFunc = PoolIconResetterFunc
    local iconPool = ns.CreateFramePool("Frame", hdr, template, resetterFunc)
...
function NugKeyFeedback:CreateAnchor()
    ...
    f:SetScript("OnDragStop",function(self)
        ...
        local db = NugKeyFeedback.db
        db.point, db.x, db.y = select(3, self:GetPoint(1))
    end)
    return f
end
